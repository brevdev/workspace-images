...
main() {
  ...
  if [ "${RSH_ARGS-}" ]; then
    ...
    curl -fsSL https://raw.githubusercontent.com/brevdev/code-server/main/install.sh | prefix "$RSH_ARGS" "$RSH" "$RSH_ARGS" sh -s -- "$ALL_FLAGS"
    return
  fi
...
install_macos() {
  if command_exists brew; then
    echoh "Installing from Homebrew. WARNING: This is not a Brev deployment, this is from coder."
    echoh
